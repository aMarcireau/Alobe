...
void Simulation::initializePopulation(shared_ptr<GraphicsWindow> graphicsWindow)
{
	...
	my_population->attachEvent(make_shared<MatingEvent>(make_unique<ConstantDistribution>((float)(BEING_MATE_RATIO))));
    ...
    for (unsigned long x = 0; x < my_land->getWidth(); ++x) {
		for (unsigned long y = 0; y < my_land->getHeight(); ++y) {
			my_land->getTile(x, y)->attachEvent(make_shared<DiseaseEvent>(
                make_unique<ConstantDistribution>((float)(DISEASE_TRANSMISSION)),
                make_unique<ConstantDistribution>((float)(DISEASE_REMISSION)),
