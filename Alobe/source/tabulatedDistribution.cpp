    ...
    for (
        ...
        if (value < distributionIterator->first) {
            if (distributionIterator == my_distribution.begin()) {
				...
				map<unsigned long, float>::iterator previousDistributionIterator(distributionIterator);
                --previousDistributionIterator;

				return (
					previousDistributionIterator->second + 
					(float)(value - previousDistributionIterator->first) / (distributionIterator->first - previousDistributionIterator->first)
					* (distributionIterator->second - previousDistributionIterator->second)
                );
            }
        }
    }
