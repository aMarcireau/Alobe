...
{
    public:
        Being(string name, multimap<string, shared_ptr<Chromosome> > chromosomes);
        ...
        multimap<string, shared_ptr<Chromosome> > getChromosomes() const;            // Getter for the being chromosomes
		...
		multimap< string, shared_ptr<Chromosome> > setChildChromosomes(Being & mate); // Create a genome for a potential child
		bool isReadyToMate(Being & mate);                                       // Test if being will mate with an other being

    private:
        string my_name;
        multimap<string, shared_ptr<Chromosome> > my_chromosomes;
